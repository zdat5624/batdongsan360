...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    Page<Transaction> findByUserIdAndAmountGreaterThanAndStatus(Long userId, long amount, TransStatusEnum status, Pageable pageable);

    @Query("SELECT t FROM Transaction t WHERE t.user.id = :userId AND t.amount < :amount AND (:status IS NULL OR t.status = :status)")
    Page<Transaction> findByUserIdAndAmountLessThanAndStatus(Long userId, long amount, TransStatusEnum status, Pageable pageable);
